...
let match = null;
let regex = /(\d+)|([RL])/g;
let lastline = data.slice(-1);
while (match = regex.exec(lastline)) {
    instructions.push(match[0]);
}

class Location {
    constructor(isWall, x, y, area) {
        this.isWall = isWall;
        this.x = x;
        this.y = y;
        this.area = area;
...
for (let y = 0; y < lines.length; y++) {
    for (let x = 0; x < lines[y].length; x++) {
        if (lines[y][x] !== " ") {
            ...
            const location = new Location(isWall, x, y, area);
            board[`${x};${y}`] = location;
...
locations.forEach(location => {
    Object.entries(directions).forEach(([dir, vector]) => {
        let neighborX = location.x + vector[0];
        let neighborY = location.y + vector[1];
        let neighborKey = `${neighborX};${neighborY}`;
        let neighbor = { loc: board[neighborKey], nextdir: dir };

        if (!neighbor.loc) {
            // gotta wrap around...
            const edgerot = edgerotations[location.area][dir];
            
            neighbor = {loc: edgerot.other(location), nextdir: edgerot.newdir};
    ...
    for (let idx = 0; idx < instructions.length; idx++) {
        if (idx % 2 === 0) {
            ...
            for (let n = 0; n < steps; n++) {
                if (location[direction].loc.isWall) break; // Hit a wall!
                const target = location[direction];
                location = target.loc; // Move one step.
                direction = target.nextdir; // Some steps require a new direction.
...
console.log("Part 2:", simulate(start));

...
function compare(one, two) {
  ...
  if (Array.isArray(one) && Array.isArray(two)) {
    return compareLists(one, two);
  }
  if (Array.isArray(one) && typeof two === 'number') {
    return compareLists(one, [two]);
  }
  if (typeof one === 'number' && Array.isArray(two)) {
    return compareLists([one], two);
  }
  console.log(one, two);
  throw new Error("Not sure how to compare these");
}

let part1 = 0;

for (let i = 0, pair = 1; i < input.length; i += 2, pair++) {
  const one = input[i];
  const two = input[i+1];
